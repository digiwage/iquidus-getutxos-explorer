...
function update_address(hash, txid, amount, type, outpoint_hash, outpoint_index, cb) {
    ...
    if (address) {
      ...
      if ( hash == 'coinbase' ) {
        ...
        lib.is_unique(address.txs, txid, function(unique, index) {
          ...
          if (unique == true) {
            tx_array.push({addresses: txid, type: type, out_hash: outpoint_hash, out_index: outpoint_index });
      ...
      if (type == 'vin') {
        var newAddress = new Address({
          a_id: hash,
          txs: [ {addresses: txid, type: 'vin', out_hash: outpoint_hash, out_index: outpoint_index } ],
          sent: amount,
          balance: amount,
        });
      } else {
        var newAddress = new Address({
          a_id: hash,
          txs: [ {addresses: txid, type: 'vout', out_hash: outpoint_hash, out_index: outpoint_index} ],
          received: amount,
          balance: amount,
        });
      }
          ...
          lib.prepare_vin(tx, function(vin) {
            lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
              lib.syncLoop(vin.length, function (loop) {
                ...
                update_address(nvin[i].addresses, txid, nvin[i].amount, 'vin', nvin[i].out_hash, nvin[i].out_index , function(){
                  loop.next();
                });
              }, function(){
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  if (vout[t].addresses) {
                    update_address(vout[t].addresses, txid, vout[t].amount, 'vout', "", t, function(){
                      subloop.next();
                    });
                  } else {
                    subloop.next();
...
module.exports = {
  ...
  // only works if out_hash, out_index is added to address.vin array (MIP patch)
  check_is_unspent: function( out_hash, out_index, cb) {
    Tx.find({'vin':{$elemMatch: {'out_index':out_index,'out_hash':out_hash}}}).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
		  if (txs) {
