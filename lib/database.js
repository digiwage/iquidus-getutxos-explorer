...
function add_to_unspent(hash, txid, index, unspent_data, cb)
{
    find_address(hash, function(address) {
      if (address) {
          ...
          arr_unspent.push( {txid: txid, vout: unspent_data.n, scriptPubKey: unspent_data.scriptPubKey.hex, satoshis: unspent_data.valueSat } );
    ...
    if (address) {
      ...
      if (type == 'vin') {
        ...
        var newAddress = new Address({
          ...
          unspent: [ {txid: txid, vout: data.n, scriptPubKey: data.scriptPubKey.hex, satoshis: data.valueSat } ],
          received: amount,
          balance: amount,
        });
      }
      ...
      lib.get_block(tx.blockhash, function(block){
        if (block) {
          lib.syncLoop(tx.vin.length, function (vinloop) {
            ...
            remove_from_unspent( tx.vin[n].txid, tx.vin[n].vout, function(ret) {
              vinloop.next();
            });
          }, function(){
